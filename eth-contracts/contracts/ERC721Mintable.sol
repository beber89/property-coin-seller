...
contract Ownable {
    ...
    modifier notContract(address a) {
        require(!Address.isContract(a), "new owner can not refer to a contract address");
        _;
    }

    function transferOwnership(address newOwner) public onlyOwner notContract(newOwner) {
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != ownerOf(tokenId), "Can not approve token transfer to the owner of the token");
        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == ownerOf(tokenId) || isApprovedForAll(ownerOf(tokenId), msg.sender), "Owner must be the caller or among approved for all list");
    ...
    function transferFrom(address from, address to, uint256 tokenId) public {
        require(_isApprovedOrOwner(msg.sender, tokenId), "address from is not approved nor owner");
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        if(_exists(tokenId)) revert("Minting tokenId already exists");
        if(to == address(0)) revert("Address is invalid");
        if(Address.isContract(to)) revert("Address is contract address");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(_tokenOwner[tokenId] == from, "Faulty transfer! Not the token owner");

        // TODO require token is being transfered to valid address
        require(to != address(0) && !Address.isContract(to), "Faulty transfer! not sending to a real address");
        // TODO clear approval
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function _setTokenURI(uint256 tokenId) internal {
        string memory tokenStrURI = strConcat(_baseTokenURI, uint2str(tokenId));
        _tokenURIs[tokenId] = tokenStrURI;
    }
...
contract PropertySellerCoin is ERC721Metadata {
